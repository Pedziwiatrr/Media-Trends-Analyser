...
def test_get_daily_summary_for_source(mock_model, mock_settings):
    agent = SummaryAgent(model=mock_model, settings=mock_settings)
...
def test_get_daily_summary_for_source_empty(mock_model, mock_settings):
    agent = SummaryAgent(model=mock_model, settings=mock_settings)
    test_date = date(2026, 1, 1)

    result = agent.get_daily_summary_for_source([], "BBC", test_date)
